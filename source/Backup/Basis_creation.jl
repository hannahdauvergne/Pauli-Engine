...
function do_basis(
    ...
    if particletype == "fermi"
        maxe = lvls + div(sum(particles .* (particles .+ 1)), 2) - maximum(particles)
        maxend = maxe - div(sum(particles[1:end-1] .* (particles[1:end-1] .+ 1)), 2) - div(particles[end] * (particles[end] - 1), 2)
        numstates = iteration_number_of_states_fermi(particles[end], internalstates, particles, maxend, maxe, 0)
        lvlstates = maxe
    elseif particletype == "bose"
        numstates = iteration_number_of_states_bose(particles[end], internalstates, particles, lvls, lvls, 0)
        lvlstates = lvls
    else
...
function iteration(
    ...
    if particlestate <= length(particles)
        if particletype == "fermi"
            ...
            basis, globaliteration = creationparticle(basis, state, particles, maxe, particletype, particlestate, globaliteration, 0, maxnow, inbasis)
        elseif particletype == "bose"
...
function creationparticle(
    ...
    if particles[particlestate] == 0
        return iteration(basis, state, particles, lvls, particletype, particlestate + 1, globaliteration, inbasis)
    end
    if particletype == "fermi"
        for pos in (particles[particlestate] - particleiteration):lastloc
            ...
            if particleiteration + 1 != particles[particlestate]
                ...
                basis, globaliteration = creationparticle(
                    basis, statei, particles, lvls - pos, particletype, particlestate, globaliteration, particleiteration + 1, min(pos - 1, lvls - pose), inbasis
                )
            else
                basis, globaliteration, inbasis = iteration(
                    basis, statei, particles, lvls - pos, particletype, particlestate + 1, globaliteration, inbasis
                )
            end
        end
    elseif particletype == "bose"
        for pos in 1:lastloc
            ...
            if particleiteration + 1 != particles[particlestate]
                basis, globaliteration = creationparticle(
                    basis, statei, particles, lvls - pos + 1, particletype, particlestate, globaliteration, particleiteration + 1, min(pos, lvls - pos + 1), inbasis
                )
            else
                basis, globaliteration, inbasis = iteration(
                    basis, statei, particles, lvls - pos + 1, particletype, particlestate + 1, globaliteration, inbasis
                )
            end
        end
    end
