###############################################################################
# STA vs Linear protocol for trap-frequency stroke (g = 0)
# Compares final energy vs stroke duration
###############################################################################
...
particles = [2]
system = Few_Particles_Hamonic_Oscillator(particles, 50, "bose")
...
Tstrokes = range(500.0, 2000.0, 30)
...
p = plot(Tstrokes, final_energy_STA,
         marker=:circle,
         label="STA",
         xlabel="Stroke duration",
         ylabel="Final energy")

plot!(p, Tstrokes, final_energy_linear,
      marker=:square,
      label="Linear")

hline!(p, [final_energy_theory],
       linestyle=:dash,
       label="Adiabatic theory")

savefig("sta_vs_linear_one_stroke.pdf")
display(plot!())
